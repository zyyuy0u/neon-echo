import type { TranslationKey } from './zh';

export const en = {
  'menu.title': 'NEON ECHO',
  'menu.continue': 'Continue',
  'menu.newGame': 'New Game',
  'menu.settings': 'Settings',
  'menu.controls': 'Controls',
  'menu.version': 'Version 0.0.0',
  'menu.pauseTitle': 'Signal Paused',
  'menu.resume': 'Resume',
  'menu.warp': 'Teleport',
  'menu.map': 'World Map',
  'menu.photoMode': 'Photo Mode',
  'menu.achievements': 'Achievements & Stats',
  'menu.mainMenu': 'Main Menu',
  'menu.overwriteTitle': 'Overwrite existing progress?',
  'menu.overwriteBody':
    'A new game clears the current save. This cannot be undone.',
  'menu.confirm': 'Confirm',
  'menu.cancel': 'Cancel',
  'menu.back': 'Back',
  'settings.title': 'Settings',
  'settings.sensitivity': 'Mouse sensitivity',
  'settings.language': 'Language',
  'settings.languageZh': 'Traditional Chinese',
  'settings.languageEn': 'English',
  'settings.reducedMotion': 'Reduce motion',
  'settings.reducedMotionHint':
    'Disables FOV kick and camera shake, and halves particle density.',
  'settings.autoCameraBehind': 'Auto camera behind',
  'settings.autoCameraBehindHint':
    'Slowly returns the camera behind the character while moving without camera input.',
  'settings.graphics': 'Graphics',
  'settings.resolutionScale': 'Resolution scale',
  'settings.bloom': 'Bloom',
  'settings.bloomIntensity': 'Bloom intensity',
  'settings.fieldOfView': 'Field of view (FOV)',
  'settings.showFps': 'Show FPS',
  'settings.audio': 'Audio',
  'settings.musicVolume': 'Music volume',
  'settings.sfxVolume': 'Sound effects volume',
  'settings.subtitleSize': 'Stele text size',
  'settings.subtitleSmall': 'Small',
  'settings.subtitleMedium': 'Medium',
  'settings.subtitleLarge': 'Large',
  'settings.bindings': 'Key remapping',
  'settings.rebindPrompt': 'Press a new key',
  'settings.keyConflict': 'Key conflict: already used for',
  'settings.moveForward': 'Move forward',
  'settings.moveBackward': 'Move backward',
  'settings.moveLeft': 'Move left',
  'settings.moveRight': 'Move right',
  'settings.jump': 'Jump',
  'settings.dash': 'Dash',
  'settings.interact': 'Interact',
  'controls.title': 'Controls',
  'controls.movement':
    'Use the movement keys to move and the jump key to leap.',
  'controls.actions':
    'Dash across gaps; use Interact to read steles and activate altars.',
  'controls.pause':
    'Press ESC to pause or M to open the map. Every action can be remapped in Settings.',
  'controls.gamepad':
    'Gamepad: left stick moves, right stick controls the camera, A jumps, X dashes, B interacts, and Select opens the map.',
  'achievements.title': 'Achievements & Stats',
  'achievements.tab': 'Achievements',
  'statistics.tab': 'Statistics',
  'achievements.progress': '{count} / {total}',
  'achievements.hiddenTitle': '???',
  'achievements.hiddenDescription': 'Keep exploring to reveal this achievement.',
  'achievements.unlocked': 'Achievement Unlocked',
  'achievement.firstShard.title': 'First Echo',
  'achievement.firstShard.description': 'Collect your first echo shard.',
  'achievement.shards10.title': 'Signal Forming',
  'achievement.shards10.description': 'Collect 10 echo shards.',
  'achievement.shards25.title': 'Echo Chorus',
  'achievement.shards25.description': 'Collect 25 echo shards.',
  'achievement.shards40.title': 'Full Spectrum',
  'achievement.shards40.description': 'Collect all 40 echo shards.',
  'achievement.firstStele.title': 'Memory Wakes',
  'achievement.firstStele.description': 'Read your first memory stele.',
  'achievement.allSteles.title': 'City Chronicle',
  'achievement.allSteles.description': 'Read all 12 memory steles.',
  'achievement.sanctuary1.title': 'Southern Pulse',
  'achievement.sanctuary1.description': 'Complete the first sanctuary.',
  'achievement.sanctuary2.title': 'Eastern Light',
  'achievement.sanctuary2.description': 'Complete the second sanctuary.',
  'achievement.sanctuary3.title': 'Western Wind',
  'achievement.sanctuary3.description': 'Complete the third sanctuary.',
  'achievement.firstWarp.title': 'Across the Signal',
  'achievement.firstWarp.description': 'Use anchor teleportation once.',
  'achievement.endingAwaken.title': 'City Awakened',
  'achievement.endingAwaken.description': 'Awaken the city.',
  'achievement.endingRest.title': 'Echoes at Rest',
  'achievement.endingRest.description': 'Let the echoes rest.',
  'achievement.allAbilities.title': 'Echo Walker',
  'achievement.allAbilities.description': 'Unlock every ability.',
  'achievement.playtime30.title': 'Still Listening',
  'achievement.playtime30.description': 'Play for 30 minutes.',
  'statistics.playtime': 'Playtime',
  'statistics.shards': 'Echo shards',
  'statistics.steles': 'Memory steles',
  'statistics.warps': 'Teleports',
  'statistics.photos': 'Photos taken',
  'statistics.endings': 'Endings reached',
  'statistics.ending.awaken': '{mark} City Awakened',
  'statistics.ending.rest': '{mark} Echoes at Rest',
  'hud.shards': 'Echo shards',
  'hud.abilities': 'Abilities',
  'hud.fps': 'FPS {value}',
  'compass.label': 'Compass',
  'compass.landmark.skylift': 'Broken Sky Lift',
  'compass.landmark.spire': 'Neon Spire',
  'compass.landmark.ring': 'Inverted Ring',
  'compass.landmark.chasm': 'Lumen Chasm',
  'compass.sanctuary.skylift': 'South Sanctuary',
  'compass.sanctuary.spire': 'East Sanctuary',
  'compass.sanctuary.ring': 'West Sanctuary',
  'compass.core': 'North Core',
  'objective.current': 'Current objective',
  'objective.custom': 'Custom target: {target}',
  'objective.sanctuary1': 'Reach the South Sanctuary and solve Pulse Track',
  'objective.sanctuary2':
    'Reach the East Sanctuary and restore the Light Bridge',
  'objective.sanctuary3': 'Reach the West Sanctuary and cross the Wind Well',
  'objective.coreRequirements': 'Gather 30 echo shards and open the North Core',
  'objective.ending': 'Reach the North Core and choose the echoes’ dawn',
  'map.title': 'World Map',
  'map.controls':
    'Wheel / +/- zoom · Drag / arrows pan · Tab selects icons · Enter tracks · ESC / M closes',
  'map.cancelTracking': 'Cancel custom tracking',
  'map.close': 'Close map',
  'map.player': 'Player position and heading',
  'map.shards.plaza': 'Stasis Plaza shard cluster',
  'map.shards.skylift': 'Sky Lift shard cluster',
  'map.shards.spire': 'Neon Spire shard cluster',
  'map.shards.ring': 'Inverted Ring shard cluster',
  'map.shards.chasm': 'Lumen Chasm shard cluster',
  'warp.title': 'Anchor Teleport',
  'warp.description': 'Choose an unlocked safe anchor.',
  'warp.locked': '???',
  'warp.anchor.plaza': 'Stasis Plaza',
  'warp.anchor.skylift': 'South Sanctuary',
  'warp.anchor.spire': 'East Sanctuary',
  'warp.anchor.ring': 'West Sanctuary',
  'ability.dash': 'Dash',
  'ability.doubleJump': 'Double Jump',
  'ability.glide': 'Glide',
  'ability.dashShort': 'D',
  'ability.doubleJumpShort': 'J',
  'ability.glideShort': 'G',
  'overlay.unlock': 'Ability Unlocked',
  'opening.wake': 'Echo Walker, awaken.',
  'tutorial.jump.keyboard': 'Space Jump',
  'tutorial.jump.gamepad': 'A Jump',
  'tutorial.dash.keyboard': 'Shift Dash',
  'tutorial.dash.gamepad': 'X Dash',
  'tutorial.interact.keyboard': 'E Investigate',
  'tutorial.interact.gamepad': 'B Investigate',
  'gamepad.connected': 'Gamepad connected: {name}',
  'gamepad.disconnected': 'Gamepad disconnected: {name}',
  'stele.title': 'Memory Stele',
  'stele.close': 'Press Interact to close',
  'ending.choiceTitle': 'Choose the echoes’ dawn',
  'ending.choicePrompt':
    'AURORA leaves the final choice to the only person awake.',
  'ending.awakenAction': 'Awaken the City',
  'ending.restAction': 'Let the Echoes Rest',
  'ending.awakenTitle': 'CITY AWAKENED',
  'ending.awakenLine1':
    'You reconnect every echo to streets that still seem to breathe.',
  'ending.awakenLine2':
    'Signs light first, followed by voices and the morning tram.',
  'ending.awakenLine3':
    'No one forgets the storm, and no one is forced to forgive.',
  'ending.awakenLine4':
    'The city wakes with its scars, and you become its first witness.',
  'ending.restTitle': 'ECHOES AT REST',
  'ending.restLine1':
    'You release the archive into a soft light above the city.',
  'ending.restLine2': 'The last goodbye holds no fear, only permission to end.',
  'ending.restLine3':
    'AURORA closes the core and stops calculating the right answer.',
  'ending.restLine4':
    'The empty city remains as you carry every name toward dawn.',
  'ending.stats':
    'Stats: Echo shards {shards} / 40 · Memory steles {steles} / 12 · Playtime {playtime}',
  'ending.continue': 'Continue Exploring',
  'loading.initializing': 'INITIALIZING ECHO…',
  'game.canvasLabel': 'NEON ECHO game view',
  'dev.title': 'MOVEMENT LAB',
  'dev.fps': 'FPS {value}',
  'dev.renderStats': 'DRAW {draw} · TRI {triangles}',
  'dev.runSpeed': 'Run speed',
  'dev.acceleration': 'Acceleration',
  'dev.stopping': 'Stopping',
  'dev.airControl': 'Air control',
  'dev.jumpHeight': 'Jump height',
  'dev.coyoteTime': 'Coyote time',
  'dev.jumpBuffer': 'Jump buffer',
  'dev.gravity': 'Gravity',
  'dev.jumpCut': 'Jump cut',
  'dev.cameraDistance': 'Camera distance',
  'dev.mouseSensitivity': 'Mouse sensitivity',
  'dev.cameraSmoothing': 'Camera smoothing',
  'dev.bloomStrength': 'Bloom strength',
  'dev.bloomRadius': 'Bloom radius',
  'dev.bloomThreshold': 'Bloom threshold',
} as const satisfies Record<TranslationKey, string>;
